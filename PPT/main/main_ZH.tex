...
\begin{frame}{About Me}
  \begin{itemize}
    \item Junior from Shanghai University of Finance and Economics majoring in Applied Mathematics (Elite Program), Major GPA (3.64/4) \\ \
    \item Boardly interested in deep learning, machine learning and scientific computing. \\ \
    \item Have done many researches about NLP. One earlier paper accepted about scientific computing.
  \end{itemize}
\end{frame}

\section{Research Projects}
\subsection{Question Answering (Feb 2020 - Jun 2020)}

\begin{frame}{Background}
  \begin{itemize}
    \item Leader of the project. Taking DL class with juniors from from Elite Program in Department of Electrical Engineering in my second year.
  \end{itemize}
\end{frame}

\begin{frame}{Data description}
  \begin{itemize}
    \item The Natural Questions (NQ) (\textbf{Kwiatkowski et al., 2019}) dataset from Google AI.
    \item Each example comprised of a google query and a corresponding Wikipedia page.
  \end{itemize}
  \begin{figure}[htbp]
    \centering
    \includegraphics[width=.7\textwidth]{figure/aa.png}
...
\subsection{Kaggle Sentence Classification (Nov 2020 - Dec 2020)}
\begin{frame}{Overview}
  \begin{itemize}
    \item Data: complaints from citizens. The task is to predict label (200 classes in total).
    \item Pre-processed the dateset, pre-trained models on similar dataset THUCNews with mixed precision training.
    \item Focal loss to track difficult and rare class examples.
    \item Designed a auxiliary sentence pair task.
    \item Also tried adversarial training, data augmentation, adding other layers after BERT, using RoBERTa-wwm, ERNIE, etc.
  \end{itemize}
\end{frame}
...
\begin{frame}{Auxiliary Task}
  \begin{itemize}
    ...
    \item Sort predicted labels by probabilities, 79\% of the true label are at the first place, 11\% at the second, 96\% of them are within top 5 predictions. \\ \
  \end{itemize}
  \begin{figure}[htbp]
    \centering
...
\begin{frame}{Final Submission}
  \begin{itemize}
    \item Highest public score in class (led by 0.4\%) but turned out to be overfitted. (why? lack of cross validation?)
  \end{itemize}
  \begin{table}[htbp]
    \centering
...
\subsection{Finite Element Method (Jul 2020 - Feb 2021)}

\begin{frame}{Biref Introduction}
  Finite element space:
  \begin{figure}[htbp]
    \centering
    \includegraphics[width=.7\textwidth]{figure/basis.png}
    \caption{Linear basis in 1D}
    \label{fig:}
  \end{figure}
...
\begin{frame}{Biref Description}
  \begin{itemize}
    \item Can be solved efficiently with the simplest Morley element.
  \end{itemize}
  \begin{figure}[htbp]
    \centering
...
\subsection{Interpreting NLP Model (Jan 2021 - Present)}
...
\begin{frame}{Background}
  ...
  Trying to explain what the model is learning during the training process.
    ...
    {\tiny
      $$
        \begin{aligned}
          Loss\left(x_{1}, \ldots, x_{d}\right)= & \sum_{n_{1}=0}^{\infty} \cdots \sum_{n_{d}=0}^{\infty} \frac{\left(x_{1}-a_{1}\right)^{n_{1}} \cdots\left(x_{d}-a_{d}\right)^{n_{d}}}{n_{1} ! \cdots n_{d} !}\left(\frac{\partial^{n_{1}+\cdots+n_{d}} f}{\partial x_{1}^{n_{1}} \cdots \partial x_{d}^{n_{d}}}\right)\left(a_{1}, \ldots, a_{d}\right)                        \\
          =                                   & f\left(a_{1}, \ldots, a_{d}\right)+\sum_{j=1}^{d} \frac{\partial f\left(a_{1}, \ldots, a_{d}\right)}{\partial x_{j}}\left(x_{j}-a_{j}\right)+\frac{1}{2 !} \sum_{j=1}^{d} \sum_{k=1}^{d} \frac{\partial^{2} f\left(a_{1}, \ldots, a_{d}\right)}{\partial x_{j} \partial x_{k}}\left(x_{j}-a_{j}\right)\left(x_{k}-a_{k}\right) \\
...
\begin{frame}{Summary}
  \begin{itemize}
    \item Comfortable with math and can implement experiments fast. \\ \
    \item Highly motivated and always passionate to search for and think of more techniques. \\ \
    \item Seeking for further guidance :) .
  \end{itemize}
\end{frame}
